  ...
  try {
    ...
    for (const a of articles) {
      ...
      const values = {
        ...
        summary: a.summary,
        content: a.content, // clean text — source lives in its own fields now
      ...
      const existing = await Article.findOne({ where: { title: a.title } });
      if (existing) {
        await existing.update(values);
        updated++;
      } else {
        await Article.create({ title: a.title, ...values });
        created++;
      }
    }
